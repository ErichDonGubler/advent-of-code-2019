    ...
    fn run_intcode_machine(mut slots: &mut [u32]) -> Result<(), AnyhowError> {
        ...
        loop {
                ...
                F: Fn(u32, u32, &mut u32) -> Result<(), AnyhowError>,
            {
                ...
                .with_context(|| {
                    anyhow!("failed executing instruction at 0x{:X}", program_counter)
                })?;
                        ...
                        *mut_coord = step(*mut_coord).unwrap();
                        ...
                        if let (true, true) = record {
                            println!(
                                "Crossing: {:?}",
                                (
                                    isize::try_from(curr_x).unwrap()
                                        - isize::try_from(origin_adjusted_x).unwrap(),
                                    isize::try_from(curr_y).unwrap()
                                        - isize::try_from(origin_adjusted_y).unwrap(),
                                )
                            );
